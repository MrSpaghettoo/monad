    ...
    */
    ...
    if (0 != memcmp(
        ...
        if (io->storage_pool().chunks(storage_pool::cnv) > 1) {
            ...
            // Allocate the configured number of cnv chunks - 1 for root offsets
            auto const max_chunks_to_allocate =
                io->storage_pool().chunks(storage_pool::cnv) - 1;
            for (uint32_t n = 2; n <= max_chunks_to_allocate; n++) {
