...
Result<void> process_monad_block(
    ...
    BlockHashChain &block_hash_chain,
    ...
    BlockCache &block_cache, bool const is_first_block)
{
...
Result<std::pair<uint64_t, uint64_t>> runloop_monad_ethblocks(
    ...
    Db &db, vm::VM &vm, BlockHashBufferFinalized &block_hash_buffer,
    fiber::PriorityPool &priority_pool, uint64_t &finalized_block_num,
    uint64_t const end_block_num, sig_atomic_t const volatile &stop,
    bool const enable_tracing, monad_chain_config const chain_config)
{
    ...
    // Pre-populate cache with parent and grandparent blocks if starting from non-zero block
    if (block_num > 0) {
        uint64_t const start_cache_block =
            block_num > 2 ? block_num - 2 : 0;
        for (uint64_t i = start_cache_block; i < block_num; ++i) {
            Block block;
            MONAD_ASSERT_PRINTF(
                block_db.get(i, block, chain_config),
    ...
    while (block_num <= end_block_num && stop == 0) {
        Block block;
        MONAD_ASSERT_PRINTF(
            block_db.get(block_num, block, chain_config),
        ...
        if (block_num > 2 && block_num % batch_size == 0) {
            std::erase_if(
                block_cache,
                [block_num](std::pair<bytes32_t, BlockCacheEntry> const &entry) {
